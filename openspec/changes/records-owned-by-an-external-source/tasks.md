...
- **files**: `lib/Service/SynchronizationService.php`, `lib/Db/SynchronizationContract.php`, `lib/Service/ObjectService.php`
- [ ] Implement (mode, source, origin identifier, last-seen; `local` for an unmaintained object; unknown before the first run)
- [ ] Test

### Task 2: The ownership mode is declared on the synchronisation
- **spec_ref**: `openspec/changes/records-owned-by-an-external-source/specs/source-owned-records/spec.md#requirement-a-record-maintained-from-a-source-says-who-owns-it-req-sor-001`
- **files**: the synchronisation schema in `lib/Settings/`, the synchronisation edit modal, Dutch and English strings
- [ ] Implement
- [ ] Test
...
- **files**: `lib/Service/SynchronizationService.php`, the synchronisation schema in `lib/Settings/`
- [ ] Implement (`delete` default, `markEnded`, `keepAndFlag`, refusal at save on an unknown value)
- [ ] Test
...
- **files**: `lib/Service/SynchronizationService.php`, the run log
- [ ] Implement (end date, absence marking, both timestamps, the flag cleared on return, counts reported)
- [ ] Test
...
- [ ] Implement (one gate for all three policies, the skip recorded with its reason)
- [ ] Test (a truncated fetch, a 429, an incremental run, and the ratio guard)
...
- **files**: `lib/Service/ObjectService.php`, the object delete surface, Dutch and English strings
- [ ] Implement (refusal naming the synchronisation, override with reason, user and timestamp, refusal on an empty reason)
- [ ] Test
...
- **files**: `lib/Service/`, `appinfo/routes.php`, `docs/`
- [ ] Implement
- [ ] Test

## Verification

- [ ] `openspec validate records-owned-by-an-external-source --strict` passes
- [ ] PHPUnit run in the container, exit code read rather than the summary line
- [ ] A run against a fixture source that drops a record under each of the three policies, with the run counts read back
