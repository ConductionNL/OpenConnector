...
- [ ] Implement (the job stores the data hash, never the data)
- [ ] Test
...
- [ ] Implement (activation refused without `credentialRef`; mock mode with fixtures)
- [ ] Test
...
- [ ] Implement
- [ ] Test (resolution, and a log assertion that no key is written)
...
- [ ] Implement
- [ ] Test (`tests/e2e/document-generation-source.spec.ts`, `tests/e2e/connector-catalog.spec.ts`)

## Verification

- [ ] `openspec validate document-generation-vendor-adapter --strict` passes
- [ ] PHPUnit run in the container, exit code read rather than the summary line
- [ ] No credential string appears in any method signature, source configuration or app-config key added by this change
