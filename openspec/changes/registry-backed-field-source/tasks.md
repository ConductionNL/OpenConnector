...
- [ ] Implement (`suggest`, `resolve`, `describe`; DI tag discovery; first-wins collision policy as `IntegrationRegistry`)
- [ ] Test (an unknown provider id fails naming itself and returns nothing)
...
- [ ] Implement (a suggestion is resolved by its identifier before it is stored)
- [ ] Test
...
- [ ] Implement (provider id, source identifier, read timestamp, cache age; `manual` for a typed value)
- [ ] Test (a typed value carries no provider id; provenance is a field and not a description)
...
- [ ] Implement (budget per provider, cache bypass on demand, no value described as live past its budget)
- [ ] Test
...
- [ ] Implement (last known value with its age and an unreachable state; never a blank, never a silent stale value)
- [ ] Test (unreachable with a cache, and unreachable with nothing cached)
...
- [ ] Implement (each over its existing source through `CallService`; no new HTTP client, per ADR-005 and ADR-011)
- [ ] Test (a binding with no source fails before any HTTP call)
...
- [ ] Implement (last resync shown, change count reported, a failed resync keeps the previous list)
- [ ] Test
...
- [ ] Test (`tests/e2e/registry-backed-field-source.spec.ts`, `openspec validate registry-backed-field-source --strict`)
