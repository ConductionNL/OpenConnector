...
- [ ] Implement (subject reference, channel, one row per recipient, ordered steps, failure naming its step and the transport's reason, a record even when the first step fails)
- [ ] Test (a partial failure across three recipients, and a pre-transport failure)
...
- [ ] Implement (a permission distinct from seeing that a send happened, redaction before the write, the read itself recorded)
- [ ] Test (a handler without the permission, and a credential that must not reach storage)
...
- [ ] Implement (single and bulk, a new attempt appended to the same record, per-item outcomes, no second retry mechanism)
- [ ] Test (a bulk retry where two items fail again)
...
- [ ] Implement (body and attachments carried, a linked record, the original untouched, the link readable from both ends)
- [ ] Test
...
- [ ] Implement (`reported`, `not reported`, `unsupported by this channel`; no inference from silence; no read inferred from a delivery)
- [ ] Test (a reporting channel, a silent one, and one with no read reporting)
...
- [ ] Implement (a query over the log, never a field written onto another app's record; a distinct never-contacted answer)
- [ ] Test
...
- [ ] Implement (an address with no account, same statuses and steps, no account created, no silent drop)
- [ ] Test (a refused external address leaves the other recipients unaffected)
...
- [ ] Record that cluster 60 adds sender identity as a field on this record once it is re-read under D12
- [ ] Test (`tests/e2e/outbound-message-log.spec.ts`, `openspec validate outbound-communication-log --type change --strict`)
