...
- [ ] Implement (`receive`, `describe`, `reply`; the normalised inbound shape with channel id, correspondent, text, attachments, optional location and the raw payload; first-wins collision policy as `IntegrationRegistry`)
- [ ] Test (an unknown channel id fails naming itself and creates nothing)
...
- [ ] Implement (channel plus optional condition plus target; a reviewable inbox for an unmatched message; no default case; per-item isolation per `synchronization-engine` REQ-008)
- [ ] Test (an unmatched message held with its reason, and a batch with one unparseable item)
...
- [ ] Implement (signature verified before the body is read, field mapping as configuration, a rejected submission recorded, a mapping onto a missing field refused at save)
- [ ] Test (Newman for a signed and an unsigned submission)
...
- [ ] Implement (location written to the mapped field, media held as attachments, nothing written when the channel supplies nothing)
- [ ] Test (a report with coordinates and photos, and a channel with neither)
...
- [ ] Implement (`reply()` on the arriving channel, `unsupported by this channel` where it cannot, no silent fallback, every reply recorded)
- [ ] Test
...
- [ ] Record C-intake-4 as belonging to the Nextcloud platform programme under D9, and C-intake-12 and C-tasks-and-phases-31 as a product decision with documented passers only under D21
- [ ] Test (`tests/e2e/intake-channels.spec.ts`, `openspec validate intake-channels-beyond-mail --type change --strict`)
